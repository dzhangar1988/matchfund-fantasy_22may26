...
Deno.serve(async (req) => {
  try {
    ...
    // Idempotency guard — only act on funds still "open".
    // A fund already cancelled/closed/finished is skipped so refunds never fire twice.
    if (fund.status !== 'open') {
      return Response.json({ ok: true, skipped: true, reason: `fund is ${fund.status}` });
    }
