...
const navigationItems = [
  ...
  { title: "Создать фонд", url: createPageUrl("CreateFund"), icon: Plus },
  { title: "Лидеры", url: createPageUrl("Leaderboard"), icon: Crown },
  { title: "FAQ", url: createPageUrl("FAQ"), icon: HelpCircle },
  { title: "Профиль", url: createPageUrl("Profile"), icon: UserIcon },
];

const adminNavigationItems = [
  { title: "Управление матчами", url: createPageUrl("AdminMatches"), icon: Settings, adminOnly: true },
];
      ...
      {showUsernameModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#0F1E35] border border-gray-800 rounded-2xl p-8 max-w-md w-full">
            <h2 className="text-2xl font-bold text-white mb-4">Выберите имя пользователя</h2>
            <p className="text-gray-400 mb-6">
              Это имя будет отображаться в таблице лидеров и фондах
            </p>
            <input
              type="text"
              value={username}
            ...
            {user && (
              <div className="mb-6 p-4 rounded-xl bg-gradient-to-br from-orange-500/10 to-yellow-500/10 border border-orange-500/20">
                ...
                <div className="text-3xl font-bold text-white">
                  ...
                  <span className="text-sm text-orange-400 ml-2">баллов</span>
                </div>
                {user.is_admin && (
